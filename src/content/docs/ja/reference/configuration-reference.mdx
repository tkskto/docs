...
プリレンダリングされたページの末尾スラッシュは、ホスティングプラットフォームによって処理され、選択した設定を尊重しない場合があります。詳細については、ホスティングプラットフォームのドキュメントを参照してください。現時点では、このユースケースにAstroの[リダイレクト](#redirects)を使用することはできません。
